...
if (strpos($query, "_") !== false) {
  $userLoggedIn = mysqli_query($connection, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
  ...
  while($row = mysqli_fetch_array($usersReturned)) {
    ...
    if($user->isFriend($row['username'])) {
      echo "<div class='resultDisplay'>
       <a href='messages.php?user=" . $row['username'] . "' style='color: #000'>
